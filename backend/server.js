...
// Map removed in favor of MongoDB
// const polls = new Map();
...
io.on('connection', (socket) => {
  ...
  // Helper to remove vote if this is the last client for the IP
  const removeVoteIfLastClient = async (pollId, socket) => {
      const userIP = socket.handshake.address;
      const roomName = `poll_${pollId}`;
      const room = io.sockets.adapter.rooms.get(roomName);
      
      // We can't strictly rely on room size here because socket might be leaving or disconnecting
      // But we can check if any *other* socket in the room has the same IP
          ...
          try {
            ...
            if (poll) {
                 ...
                 if (voteEntry) {
                     ...
                     console.log(`Vote removed for ${userIP} in poll ${pollId} (User left)`);
                 }
            }
          } catch (err) {
  ...
  socket.on('create_poll', async (data) => {
    ...
    try {
        ...
        socket.emit('poll_created', { pollId, pollData: { ...newPoll.toObject(), userVotes: undefined } });
